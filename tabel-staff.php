...
session_check_endpoint_logged_only("tabel-staff.php");
...
try {
  ...
  $stmt->bind_result($id, $employee_number, $name, $role_name, $address, $last_edu, $teaching_history);
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100;200;300;400;500;600&display=swap"
    rel="stylesheet">
  <title>Tabel Data Staff dan Guru | SMAN 5 Malang</title>
    ...
    <div class="w-full max-w-7xl mx-auto px-4 md:px-12 lg:px-4 mt-12">
      ...
      <div class="mt-12 p-1 flex items-end gap-3 text-sm font-semibold">
        <div class="flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
          ...
          </svg> <span>&nbsp;Tabel Data</span></div> <span>></span> <span>Staff & Guru</span>
      </div>
      <div class="mt-12">
        ...
        if (isset($_GET["msg"])) {
          ...
          switch ($msg) {
            case "postSuccess":
              echo '
              <p class="text-sm text-red-500">Data staff/guru berhasil disimpan</p>
              ';
              break;
            case "editSuccess":
              echo '
                <p class="text-sm text-red-500">Data staff/guru berhasil diubah</p>
                ';
              break;
            case "deleteSuccess":
              echo '
                <p class="text-sm text-red-500">Data staff/guru berhasil dihapus</p>
                ';
              break;
          }
        }
            ...
            <tbody>
              ...
              foreach ($employee as $emp) {
                ...
                <tr class="bg-white border-b  dark:border-gray-700 hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4">
                      ' . $emp["teaching_history"] . '
                  </td>
                  ' . ($_SESSION["role_id"] == 1 ? '
                  <td class="px-6 py-4 text-right align-middle">
                    <a href="edit-data-staff.php?id=' . $emp["id"] . '" class="font-medium text-blue-600 hover:underline">Edit</a>
                    <a href="./scripts/delete_data_staff.php?id=' . $emp["id"] . '" class="font-medium text-red-600 hover:underline">Hapus</a>
                  </td>
                ' : '') . '</tr>';
              }
              ?>
